...
class SemanticModelIngestRequest(HexBaseModel):
    ...
    verbose: bool = Field(
        True,
        description="Whether to respond with detail on synced components"
    )
    debug: bool = Field(
        False,
...
class SemanticModelDebugInfo(HexBaseModel):
    ...
    metricflow_model_schemas: Optional[MetricflowModelSchemas] = Field(
...
class SemanticModelsSyncResponse(HexBaseModel):
    ...
    debug: Optional[SemanticModelDebugInfo] = None